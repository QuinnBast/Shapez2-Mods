...
if [ -z "$CONTENT_PATH" ]; then
  INSTALLED="${SPZ2_PERSISTENT:-}/mods/PredictableBeltFilters"
...
#
# This mod is just a manifest and a dll - no translations, no loose resources - so those two
# are the whole check. manifest.json is not a compile output, so a mis-set OutputPath can drop
# it while still producing a dll, and a mod folder without one is silently not a mod.
for required in manifest.json PredictableBeltFilters.dll; do
