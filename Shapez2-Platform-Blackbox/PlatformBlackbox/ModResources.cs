...
/// <c>Assembly.Load(byte[])</c> - <c>LoadFrom</c> binds by assembly identity and would just hand
/// back the copy already loaded, which is the whole reason reloading needs the byte[] overload -
/// and an assembly loaded from a byte array has no file backing it, so <c>Location</c> is the
/// empty string and <c>GetParent</c> throws
...
///
/// This is the third copy of this in the repo - Crossover Platforms and Train Cargo Tools have
/// their own. Worth folding into shared source next to the toolbar kit if a fourth appears.
/// </summary>
internal static class ModResources
{
    /// Matches the project name, which is what both output paths are named after.
    private const string ModFolderName = "PlatformBlackbox";

    private const string ResourcesFolder = "Resources";

    public static ModFolderLocator Locate(ILogger logger)
    {
        string assemblyLocation = typeof(PlatformBlackboxMod).Assembly.Location;
        if (!string.IsNullOrEmpty(assemblyLocation))
        {
            return ModDirectoryLocator.CreateLocator<PlatformBlackboxMod>()
        ...
        logger.Warning?.Log($"No Resources folder found for {ModFolderName}; icons will not load");
