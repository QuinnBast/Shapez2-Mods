...
if [ -z "$CONTENT_PATH" ]; then
  INSTALLED="${SPZ2_PERSISTENT:-}/mods/TrainCargoTools"
...
# and Resources carries every toolbar icon, research and wiki image and all sixty-odd track
# meshes - FileTextureLoader and the model importer read them off disk at runtime, so without
# it the belts, junctions and lifts are all invisible. Resources is a directory, hence -e
# rather than -f.
for required in manifest.json TrainCargoTools.dll translations.json Resources; do
