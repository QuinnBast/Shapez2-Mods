...
if [ -z "$CONTENT_PATH" ]; then
  INSTALLED="${SPZ2_PERSISTENT:-}/mods/FirstPerson"
...
#
# translations.json is as load-bearing as the dll: it is not a compile output, so a mis-set
# OutputPath drops it while still producing a dll, and a missing one aborts the whole game's
# mod load in LoadModTranslationAfterResolve. It also carries the research node titles and
# the keybinding names, so without it the settings screen shows raw ids.
for required in manifest.json FirstPerson.dll translations.json; do
...
# steamcmd treats a missing preview as a warning and publishes anyway, leaving the item with
# Steam's grey placeholder. There is no art checked in for this mod on purpose - a borrowed
# screenshot would be worse than none - so this refuses rather than publishing a blank page.
if [ ! -f "$SCRIPT_DIR/preview.png" ]; then
  echo "error: Steam/preview.png is missing." >&2
  echo "       Add a preview image before publishing - steamcmd will happily publish" >&2
  echo "       without one and leave the workshop page showing a grey placeholder." >&2
  exit 1
fi
