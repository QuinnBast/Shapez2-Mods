...
if [ -z "$CONTENT_PATH" ]; then
  INSTALLED="${SPZ2_PERSISTENT:-}/mods/DecorationBlocks"
...
# and a missing Resources takes every block with it - Resources/Textures is the whole atlas,
# packed off disk at load, so without it there is nothing to draw and no toolbar icons
# either. Resources is a directory, hence -e rather than -f.
for required in manifest.json DecorationBlocks.dll translations.json Resources; do
...
# steamcmd treats a missing preview as a soft problem: it reports Success and leaves the item
# with no image, which is only visible on the Workshop page afterwards. Checked here so a first
# publish cannot quietly go out without one.
if [ ! -f "$SCRIPT_DIR/preview.png" ]; then
  echo "error: no preview.png in $SCRIPT_DIR" >&2
  echo "       The Workshop needs a preview image. Capture the mod in game and save it there." >&2
  exit 1
fi
...
BEFORE_ID=$(sed -n 's/.*"publishedfileid"[ 	]*"\([0-9]*\)".*/\1/p' "$BASE_VDF" | head -1)
